...
app.post("/auth/change-password", requireAuth, async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: "Token inválido (sem sub)" });
...
app.get("/me/school", requireAuth, async (req, res) => {
    try {
        const schoolId = req.user?.schoolId;
        if (!schoolId)
            return res.status(400).json({ error: "schoolId não encontrado no token" });
 ...
 */
app.patch("/me/school", requireAuth, async (req, res) => {
    try {
        const schoolId = req.user?.schoolId;
        if (!schoolId)
            return res.status(400).json({ error: "schoolId não encontrado no token" });
...
app.patch("/schools/:schoolId", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        if (!schoolId)
            return res.status(400).json({ error: "schoolId inválido" });
        const { name, responsible, email, phone, royaltiesFee, payDay, cep, city, uf, neighborhood, } = req.body;
        ...
        if (!cleanResponsible)
            return res.status(400).json({ error: "responsible é obrigatório" });
...
app.post("/schools", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        if (!name || !String(name).trim())
            return res.status(400).json({ error: "name é obrigatório" });
        if (!responsible || !String(responsible).trim())
            return res.status(400).json({ error: "responsible é obrigatório" });
...
app.patch("/schools/:schoolId/users/:userId", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const { name, email, phone, tempPassword, isAdmin, newPassword } = req.body;
        ...
        const sentAnyField = cleanName !== undefined ||
        ...
        let passwordHashToSet = undefined;
        if (cleanNewPassword && cleanNewPassword.length > 0) {
            passwordHashToSet = await bcryptjs_1.default.hash(cleanNewPassword, 10);
        }
        const updated = await prisma.user.update({
            where: { id: userId },
            data: {
